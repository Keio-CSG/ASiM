...
class ASiMLinear(nn.Linear):
    ...
    Bit-wise simulation Linear Module.
        ...
        non_linear_alpha: non-linear factor alpha
        weight_var: local variation of weight cell
    ...
    def __init__(self,
        ...
        self.non_linear_alpha = non_linear_alpha
    ...
    def _decompose_weight(self, input):
        ...
        # Generate variation from given sigma%
        sign_var = torch.randn(sign.shape, device=self.device) * self.weight_var / 100
        sign = sign + sign_var
        ...
        for i in range(self.wbit-2, -1, -1):
            w_idx = torch.div(input, 2.0 ** i, rounding_mode='floor')
            # Generate variation from given sigma%
            w_var = torch.randn(w_idx.shape, device=self.device) * self.weight_var / 100
            w_idx = w_idx + w_var
            w_map = torch.cat((w_map, w_idx))
    ...
    def forward(self, input):
        ...
        if self.mode == 'Simulation':   # ASiM simulation using the framework
            ...
            w_map = self._decompose_weight(w_quant)
                            ...
                            for j in range(bp_cycle-1, -1, -1):
                                ...
                                if j == bp_cycle - 1:
                                    ...
                                    if i + j <= wbit + bp_cycle - self.boundary_levels - 2:
                                        ...
                                        bp_mac = bp_mac + rand_noise
                                        bp_mac = torch.clamp(bp_mac, 0.0, 1.0)
                                    # Add Non-Linearity
                                    bp_mac = bp_mac / (1 + self.non_linear_alpha * bp_mac)
                        ...
                        for i in range(wbit-1, -1, -1):
                            ...
                            for j in range(xbit-1, -1, -1):
                                mac_col = F.linear(x_map[n][xbit-1-j].to(dtype=torch.float32),
                                                   ...
                                                   bias=None)
                                # Add Non-Linearity
                                mac_col = mac_col / (1 + self.non_linear_alpha * mac_col)
                                # Scale MAC results by full range and clamp MAC to [0, 1]
