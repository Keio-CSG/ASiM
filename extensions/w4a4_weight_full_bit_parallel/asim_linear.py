...
class ASiMLinear(nn.Linear):
    ...
    Bit-wise simulation Linear Module.
        ...
        weight_bp: using full weight bit-parallel or not
                ...
                for n in range(w_update):
                    ...
                    for i in range(wbit-1, -1, -1):
                        ...
                        for j in range(bp_cycle-1, -1, -1):
                            ...
                            # Scale MAC results by full range and clamp MAC to [-1, 1]
                            output = torch.clamp(output / ((2 ** self.xbit - 1) * (2 ** (self.wbit - 1) - 1) * self.nrow), -1.0, 1.0)
                            # Generate rand noise from given sigma%
                            rand_noise = torch.randn(output.shape, device=self.device) * self.rand_noise_sigma / 100
                            # Add noise and clamp real MAC to [-1, 1]
                            output = output + rand_noise
                            output = torch.clamp(output, -1.0, 1.0)
                            # Output MAC quantization by ADC
                            output = torch.round(output * (2 ** (self.adc_prec - 1.0) - 1.0))
                            # Transform ADC results to weight-bp result
                            output = output / (2 ** (self.adc_prec - 1.0) - 1.0) * ((2 ** self.xbit - 1) * (2 ** (self.wbit - 1) - 1) * self.nrow)
